...
class PetugasController extends MY_Protectedcontroller
{
	public function __construct(){
		...
		$this->can_read = ['a01', 'd01', 'p01', 's01'];
		$this->can_write = ['a01', 'd01', 'p01', 's01'];
	...
	public function upIndex(){
		...
		if ($this->session->user_login['role'] == "a01") {
			// var_dump(trim($this->session->user_login['npsn']));return;
			$data['sekolah'] = $this->sekolah->getAllid(trim($this->session->user_login['npsn']));
	...
	public function downIndex()
	{
		...
		if ($this->session->user_login['role'] == "a01") {
			$data['sekolah'] = $this->sekolah->getAllid(trim($this->session->user_login['npsn']));
			$data['ruang'] = $this->sekolah->getAllid(trim($this->session->user_login['npsn']));
		}
		else
		{
